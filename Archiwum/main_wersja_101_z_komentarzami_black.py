    ...
    if len(notowania) >= dlugosc:
        ...
        if len(srednie) >= 2:
            if notowania[-1] > srednie[-1] and notowania[-2] > srednie[-2]:
                print(
                    f"Kurs kontynuuje średnioterminowy trend rosnący, kurs jest nad średnią z {dlugosc} sesji"
                )
            elif notowania[-1] < srednie[-1] and notowania[-2] < srednie[-2]:
                print(
                    f"Kurs kontynuuje średnioterminowy trend spadkowy, kurs jest pod średnią z {dlugosc} sesji"
                )
            elif notowania[-1] > srednie[-1] and notowania[-2] < srednie[-2]:
                print(
                    f"Kurs przebił od dołu średnią z {dlugosc} sesji - SYGNAŁ KUPNA"
                )
            elif notowania[-1] < srednie[-1] and notowania[-2] > srednie[-2]:
                print(
...
def logika_srednich(dluzsze_srednie, dluzsza, krotsze_srednie, krotsza):
    if len(dluzsze_srednie) > 2 and len(krotsze_srednie) > 2:
        if (
            dluzsze_srednie[-1] < krotsze_srednie[-1]
            and dluzsze_srednie[-2] < krotsze_srednie[-2]
        ):
            print(
                f"Średnie - kontynuacja trendu rosnącego. Średnia {krotsza} jest nad {dluzsza}. DŁUGOTERMINOWY WZROST"
            )
        elif (
            dluzsze_srednie[-1] > krotsze_srednie[-1]
            and dluzsze_srednie[-2] > krotsze_srednie[-2]
            ...
            print(
                f"Średnie - kontynuacja trendu spadkowego. Średnia {dluzsza} jest nad {krotsza}. DŁUGOTERMINOWY SPADEK"
            )
        elif (
            dluzsze_srednie[-1] > krotsze_srednie[-1]
            and dluzsze_srednie[-2] < krotsze_srednie[-2]
            ...
            print(
                f"Krótsza średnia {krotsza} przebiła od góry średnią {dluzsza}. SYGNAŁ ZMIANY DŁUGOTERMINOWEGO TRENDU NA SPADKOWY"
            )
        elif (
            dluzsze_srednie[-1] < krotsze_srednie[-1]
            and dluzsze_srednie[-2] > krotsze_srednie[-2]
            ...
            print(
                f"Krótsza średnia {krotsza} przebiła od dołu średnią {dluzsza}. SYGNAŁ ZMIANY DŁUGOTERMINOWEGO TRENDU NA ROSNĄCY"
            )
        elif (
            dluzsze_srednie[-1] < krotsze_srednie[-1]
            and dluzsze_srednie[-2] == krotsze_srednie[-2]
            ...
            print(
                f"Średnie - trend rosnący. Średnia {krotsza} jest nad {dluzsza}. DŁUGOTERMINOWY WZROST"
            )
        elif (
            dluzsze_srednie[-1] > krotsze_srednie[-1]
            and dluzsze_srednie[-2] == krotsze_srednie[-2]
            ...
            print(
                f"Średnie - trend spadkowy. Średnia {krotsza} jest pod {dluzsza}. DŁUGOTERMINOWY SPADEK"
            )
...
def program():
    ...
    while True:
        ...
        logika_srednich(
            dluzsze_srednie=srednie_sma_21,
            dluzsza=21,
            krotsze_srednie=srednie_sma_8,
            krotsza=8,
        )
        ...
        if srednie_sma_8 and srednie_sma_21:
            print(
                f"Kurs - otwarcie: {notowania[0]}, maks: {max(notowania)}, min: {min(notowania)}, ostatni:"
            ...
            print(
                f"Kurs - otwarcie: {notowania[0]}, maks: {max(notowania)}, min: {min(notowania)},"
